        ...
        for epoch in range(args.max_epoch):
            ...
            while True:
                ...
                try:
                    ...
                    break
                iter_loss, iter_logit, _train_op = model.run(batch_data, model, sess, run_list=[_loss, train_logit, train_op], mode="train")
            ...
            if (epoch + 1)% args.test_epoch == 0:
                ...
                while True:
                    ...
                    try:
                        ...
                        break
                    iter_logit = model.run(test_data, model, sess, run_list=[train_logit], mode="test")
                    t = time.time() - time_start
                    time_sum += t
                    iter_output = iter_logit.argmax(-1)
                    ...
                    if tot_not_na > 0:
                        sys.stdout.write("&&&&&&[TEST] Epoch %d | not NA accuracy: %f, accuracy: %f\r"% (epoch, float(tot_not_na_correct) / tot_not_na, float(tot_correct) / tot))
                        sys.stdout.flush()
